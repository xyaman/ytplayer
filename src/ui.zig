...
fn isWideCharacter(ch2: []const u8, len: u3) bool {
    const ch = switch (len) {
        1 => ch2[0],
        2 => std.unicode.utf8Decode2(ch2[0..2].*) catch unreachable,
        3 => std.unicode.utf8Decode3(ch2[0..3].*) catch unreachable,
        4 => std.unicode.utf8Decode4(ch2[0..4].*) catch unreachable,
        else => unreachable,
    };
